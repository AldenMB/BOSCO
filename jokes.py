...
def generalized_knock_knock():
    ...
    # who is there?
    bosco.wait()
    bosco.say("A person's first name which also sounds like part of a phrase.").join()
    # A person's first name which also sounds like part of a phrase who?
    bosco.wait()
    bosco.say("The complete phrase.").join()
    bosco.sting().join()


def elephant_sneakers():
    bosco.say("Why was the elephant wearing sneakers?").join()
    # Why?
    bosco.wait()
    bosco.say("So that he could sneak around in the grass.").join()
    bosco.sting().join()
